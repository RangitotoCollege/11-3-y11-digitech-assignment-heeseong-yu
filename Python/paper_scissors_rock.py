...
def play():
    ...
    while playing:
        print("\n======")
        # Once in a while, it randomly resets the user frequency to prevent the
        # user from overloading with one hand and then spamming another hand.
        if random.randint(1, 10) == 10:
            user_hands_frequency = ["paper", "scissors", "rock"]
        # From what user did, it creates a list for the computer to choose from
        # by inverting the list with hands that win it.
        computer_hands_frequency = []
        ...
        user_hand = None
        # Until the user gives a valid input, it repeats asking what hand they
        # want to play.
        while user_hand not in user_hands_frequency:
            try:
                user_hand = input(
                    "What hand? \n 0 : Paper \n 1 : Scissors \n 2 : Rock \n")
                # User hand above 2 is created by the user, so avoid confusion,
                ...
                # If the user wrote the hand itself, it still accepts it
                if user_hand == "paper" or user_hand == "scissors" or user_hand == "rock":
                    break
                print("Invalid input.")
        # The hand user did is appended to the list for the computer to use the
        # winning hand more frequently, as explained above.
        user_hands_frequency.append(user_hand)
        # From the rules, it determines who wins or if it's a tie.
        winner = rules.get(
            (computer_hand, user_hand), rules.get(
                (user_hand, computer_hand), 'tie'))
        print(f"{utils.name}: {user_hand}   Computer: {computer_hand}")
        # Winning and losing message and streak, and highest streak system.
        if winner == user_hand:
            print(f"{utils.name} won!")
            streak += 1
            print(f"You have won {streak} games in a row!")
            if streak > utils.check_high_score(
