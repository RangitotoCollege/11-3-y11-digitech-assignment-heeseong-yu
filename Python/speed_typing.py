...
def play():
    ...
    while playing:
        print("\n======")
        # Separate the words file into a stripped list.
        with open("Files\\common_words.txt", "r") as f:
            typing_words_list = [word.strip() for word in f.readlines()]
        # Create a sentence by adding a random word to a blank string 10 times.
        target_sentence = ""
        for i in range(10):
        ...
        # If the user sentence matches the target sentence, calculate the record.
        if user_sentence.strip() == target_sentence.strip():
            ...
            # If the user has no high scores yet, append the name and high score.
            # Or if the user has a previous high score, update the high score.
        ...
        # Ask for the user to play the game again.
